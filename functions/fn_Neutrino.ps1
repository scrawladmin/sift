Function Get-neuIPBlocklist {
    ...
    Process {
        if ($neutrinokey) {
                    ...
                    if ($info) {
                        ...
                        if ($params) {
                            ...
                            if ($params) {
                                ...
                                if ($response) {
                                    $apiname | Select-ColorString "Neutrino" -CaseSensitive -BackgroundColor $(Get-Random 'Gray','Blue','Green','Cyan','Red','Magenta','Yellow','White')
                                    # $apiname = 'Neutrino' | Trace-word -words 'Neutrino'
            ...
            Else {
                Write-Warning "Requires Neutrino User ID" -InformationAction Continue
                return
            }
        }
        Else {
...
Function Get-neuEmailvalidate_old {
    # -emailvalidate
    param($email)
...
Function Get-neuEmailvalidate {
    [cmdletbinding()]
    [OutputType("emailvalidate")]
    param(
        [parameter(Position = 0,
            Mandatory,
            HelpMessage = "Parse, validate and clean an email address.
            Parameter	Type: email
            Optional: fix-typos boolean Automatically attempt to fix typos in the address",
            ValueFromPipeline,
            ValueFromPipelineByPropertyName)]
...
Function Get-neuEmailverify_old {
    # -emailverify
    param($email)
...
Function Get-neuEmailverify {
    [cmdletbinding()]
    [OutputType("emailverify")]
    param(
        [parameter(Position = 0,
            Mandatory,
            HelpMessage = "SMTP based email address verification. Verify real users and filter out low-quality email addresses.
                        Parameter	Type: email
                        Optional: fix-typos boolean Automatically attempt to fix typos in the address",
            ValueFromPipeline,
            ValueFromPipelineByPropertyName)]
